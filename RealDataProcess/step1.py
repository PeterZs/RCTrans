...
def run_sfm(image_dir, project_dir, same_camera=False, colmap_path='colmap'):
    ...
    print('The sfm is done, Please check the sparse model in the folder:', f'{project_dir}/sparse/0')

    # convert to txt
    cmd=[colmap_path,'model_converter',
         '--input_path',f'{project_dir}/sparse/0',
         '--output_path',f'{project_dir}',
         '--output_type','TXT']
    print(' '.join(cmd))
    subprocess.run(cmd,check=True)
